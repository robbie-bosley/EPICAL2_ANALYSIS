...
void Analyse_mTower(int run, Double_t energy) // This is the main workhorse. Any added processors will find their place somewhere within here. 'energy_in' should be given in GeV.
{
  ...
  bool SQS = false;  // true if intending to save results of event selection to a local file
  bool LQS = false;  // true if intending to use local file to perform quick event selection
  ...
  int runNumber, fileNumber, eventIndex, eventID, nHits, dataSize, eventNumberOriginal;
  ...
  outtree.Branch("eventNumber",&eventNumber,"e/I");
  outtree.Branch("eventID",&eventID,"eo/I"); //This number does not reset to 0 sometimes, it goes from 0 to nEvents
  ...
  for (int event = minEvent; event < maxEvent; event++)
      ...
      for (int hit=0; hit<nHits; hit++)
	{
	  ...
	  for (int hit = 0; hit < entries; hit++)
		...
		{	  
		  // Fill any relevant histograms, if required.
	  ...
	  if (LQS) {
	    ...
	    else selection_choice = selection;
	    switch (selection_choice) {
	    case 0:  // No Selection Performed.
	      IsGood = true;
	      break;
	    case 1:
	      IsGood = EventSelectionA(DB, CheckRejects, CheckAccepts, CheckThirdLayer, C2, C4, C6, nPixelRadiusC2, nPixelRadiusC4, nPixelBorderC6, laneNumber, laneOffset, columnsPerChip, rowsPerChip, nPixelsGap, hitsInChip, eventID);
	      break;
	    case 2:
	      IsGood = EventSelectionB(clayers, cchips, nHitsTotNew, nClustersTotNew, dlim, dcenter, minNlayerBulk, W0Bulk, thDistBulk, wBinBulk, htmpBulk, htmpLim, mcellsBulk, mcellsLim);
	      break;
	    case 3:
	      if (EventSelectionA(DB, CheckRejects, CheckAccepts, CheckThirdLayer, C2, C4, C6, nPixelRadiusC2, nPixelRadiusC4, nPixelBorderC6, laneNumber, laneOffset, columnsPerChip, rowsPerChip, nPixelsGap, hitsInChip, eventID) && EventSelectionB(clayers, cchips, nHitsTotNew, nClustersTotNew, dlim, dcenter, minNlayerBulk, W0Bulk, thDistBulk, wBinBulk, htmpBulk, htmpLim, mcellsBulk, mcellsLim)) {
		IsGood = true;
	      }
	      break;
	    case 4:
	      prepreparedRobbie = EventSelectionA(DB, CheckRejects, CheckAccepts, CheckThirdLayer, C2, C4, C6, nPixelRadiusC2, nPixelRadiusC4, nPixelBorderC6, laneNumber, laneOffset, columnsPerChip, rowsPerChip, nPixelsGap, hitsInChip, eventID);
	      prepreparedHiroki = EventSelectionB(clayers, cchips, nHitsTotNew, nClustersTotNew, dlim, dcenter, minNlayerBulk, W0Bulk, thDistBulk, wBinBulk, htmpBulk, htmpLim, mcellsBulk, mcellsLim);
	      switch (selection) {
	      case 0:
		IsGood = true;
		break;
	  ...
	  if( isFlushTree ) {
	    ...
	    if (SQS) prepreparedtree->AutoSave("FlushBaskets");
	    isFlushTree = kFALSE;
	  }
